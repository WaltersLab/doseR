...
## Installation
If installing from a local file (i.e. a _tar.gz_), you simply give 
the path to the `install.packages` argument.
...
Alternatively (Eventually??), doseR can be installed from 
Bioconductor directly using the `BiocManager` package.
...
These example data are included in the package as a list.
...
These three data elements are used to populate a new `countDat` object, 
...
manipulations required before we can create a new `countDat`object.  
...
`hmel.dat$chromosome` vector into a factor, which facilitates 
...
With library sizes now stored in the `countDat` object, we can calculate
...
indicated in the `@annotation` slot of the `countDat` object. Biological
...
subset the `countDat` object so that tests are applied 
