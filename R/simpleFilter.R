...
#' @usage simpleFilter(cD, mean_cutoff=NULL, min_cutoff=NULL, median_cutoff=
#' NULL, counts=TRUE)
...
#' object, based on a selected cutoff.
...
simpleFilter <- function(cD, mean_cutoff=NULL, min_cutoff=NULL,
...
if(counts) {
if(!is.null(mean_cutoff)) { cD <- cD[   apply(cD@data , 1,
function(x) mean(x) ) >= mean_cutoff,] }
if(!is.null(min_cutoff)) { cD <- cD[   apply(cD@data , 1,
function(x) min(x) ) >= min_cutoff,] }
if(!is.null(median_cutoff)) { cD <- cD[   apply(cD@data , 1,
function(x) median(x) ) >= median_cutoff,] }
} else {
if(!is.null(mean_cutoff)) { cD <- cD[   apply(cD@RPKM , 1,
function(x) mean(x) ) >= mean_cutoff,] }
if(!is.null(min_cutoff)) { cD <- cD[   apply(cD@RPKM , 1,
function(x) min(x) ) >= min_cutoff,] }
if(!is.null(median_cutoff)) { cD <- cD[   apply(cD@RPKM , 1,
function(x) median(x) ) >= median_cutoff,] }
}

if(is.null(mean_cutoff) && is.null(min_cutoff) && is.null(median_cutoff)) {
message ('USE SYNTAX : simpleFilter(cd.head, min_cutoff = 1.744596e-09,
counts=FALSE)')
warning ('SAMPLE SYNTAX : simpleFilter(cd.head, min_cutoff = 20,
mean_cutoff = 10, median_cutoff = 30 )')
...
message( "Filtering removed ", ori_len-nrow(cD@data), " (", pct, ")
of ", ori_len, " total loci." )
invisible(cD)
}
}
