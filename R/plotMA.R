...
#' @param samplesA Either a character vector, identifying sample set A by either replicate name or sample name, or a numerical vector giving the columns of data in the 'countDat' object that forms sample set A.
#' @param samplesB Either a character vector, identifying sample set B by either replicate name or sample name, or a numerical vector giving the columns of data in the 'countDat' object that forms sample set B.
#' @param scale If given, defines the scale on which the log-ratios will be plotted.
#' @param xlab Label for the X-axis. Defaults to "A".
#' @param ylab Label for the Y-axis. Defaults to "M".
#' @param ... Any other parameters to be passed to the plot function.
...
plotMA.CD <- function (cD, samplesA, samplesB, scale = NULL,
          xlab = "A", ylab = "M", ...)
{
  if (length(dim(cD)) > 2)
    stop("This function is currently only applicable to 2-dimensional countDat objects.")
  if (is.character(samplesA)) {
    Asamps <- which(as.character(cD@replicates) %in% samplesA)
    if (!all(samplesA %in% cD@replicates))
      Asamps <- c(Asamps, which(colnames(cD@RPKM) %in%
                                  samplesA[!(samplesA %in% as.character(cD@replicates))]))
    if (!all(samplesA %in% c(colnames(cD@RPKM), as.character(cD@replicates))))
  ...
  if (is.character(samplesB)) {
    ...
    if (!all(samplesB %in% c(colnames(cD@RPKM), as.character(cD@replicates))))
  ...
  Adata <- cD@RPKM[, samplesA]
  Bdata <- cD@RPKM[, samplesB]
# if (normaliseData) {
...
#
#plotMA.CD(cd.leg, samplesA = 'M', samplesB = 'F')
#
